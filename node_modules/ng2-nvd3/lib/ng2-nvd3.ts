...
export class nvD3 {
  ...
  updateWithOptions(options){
    ...
    for (let key in this.chart) {
      ...
      else if ((key === 'xTickFormat' || key === 'yTickFormat') && options.chart.type === 'lineWithFocusChart') continue;
      else if ((key === 'tooltips') && options.chart.type === 'boxPlotChart') continue;
      else if ((key === 'tooltipXContent' || key === 'tooltipYContent') && options.chart.type === 'scatterChart') continue;

      else if (options.chart[key] === undefined || options.chart[key] === null) continue;
